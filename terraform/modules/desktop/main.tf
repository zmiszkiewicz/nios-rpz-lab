...
# gcr.io/instruqt/guacamole container. Its resolver is pinned to the Grid
# Master's LAN1 address at boot, which is what puts it behind the RPZ.
###############################################################################
...
resource "aws_instance" "desktop" {
  ...
  user_data = templatefile("${path.module}/templates/desktop-init.ps1.tpl", {
    ...
    grid_manager_url = var.grid_manager_url
  })
